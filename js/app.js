...
var L = {
  es: {
    ...
    'cat.badge':'Catálogo','cat.aititle':'Canciones Creadas con IA','cat.aisubtitle':'Canciones cristianas reales generadas por nuestra comunidad usando inteligencia artificial','cat.createbtn':'Crear mi propia canción','cat.showall':'Ver todas',
  ...
  en: {
    ...
    'cat.badge':'Catalog','cat.aititle':'AI-Created Songs','cat.aisubtitle':'Real Christian songs generated by our community using artificial intelligence','cat.createbtn':'Create my own song','cat.showall':'View all',
    'feat.badge':'Features'
  ...
  pt: {
    ...
    'cat.badge':'Catálogo','cat.aititle':'Canções Criadas com IA','cat.aisubtitle':'Canções cristãs reais geradas pela nossa comunidade usando inteligência artificial','cat.createbtn':'Criar minha própria canção','cat.showall':'Ver todas',
    'feat.badge':'Recursos'
...
// ===== AI Songs Gallery =====
var _genreNames = {
  country:'Country', rock:'Rock', gospel:'Gospel',
  folk:'Folk', worship:'Worship', ballad:'Balada'
};
...
function filterGenre(genre) {
  ...
  var sub = document.getElementById('catalogSubtitle');
  var btnAll = document.getElementById('btnShowAll');
  if (genre) {
    if (title) title.innerHTML = '<i class="fas fa-microphone-alt" style="color:var(--secondary)"></i> ' + (_genreNames[genre]||genre);
    ...
    cards.forEach(function(c) {
      if (c.querySelector('h3') && c.querySelector('h3').textContent.toLowerCase().replace('balada','ballad') === genre) c.classList.add('genre-active');
    });
  } else {
    var lang = document.documentElement.lang || 'es';
    var titles = {es:'Canciones Creadas con IA',en:'AI-Created Songs',pt:'Canções Criadas com IA'};
    var subs = {es:'Canciones cristianas reales generadas por nuestra comunidad usando inteligencia artificial',en:'Real Christian songs generated by our community using artificial intelligence',pt:'Canções cristãs reais geradas pela nossa comunidade usando inteligência artificial'};
    if (title) title.innerHTML = '<i class="fas fa-microphone-alt" style="color:var(--secondary)"></i> ' + (titles[lang]||titles.es);
    if (sub) sub.textContent = subs[lang]||subs.es;
...
function loadAISongs(genre) {
  var grid = document.getElementById('aiSongsGrid');
  if (!grid) return;
  var url = genre ? 'api/songs.php?genre=' + encodeURIComponent(genre) : 'api/save-song.php?action=list';
  grid.innerHTML = '<div style="text-align:center;color:var(--gray);padding:2rem;grid-column:1/-1"><i class="fas fa-spinner fa-spin"></i></div>';
  fetch(url)
  ...
  .then(function(data) {
    ...
    if (songs.length === 0) {
      var lang = document.documentElement.lang || 'es';
      var msgs = {es:'Aún no hay canciones de este género. ¡Sé el primero en crear una!',en:'No songs in this genre yet. Be the first to create one!',pt:'Ainda não há canções deste gênero. Seja o primeiro a criar uma!'};
      grid.innerHTML = '<div style="text-align:center;color:var(--gray);padding:2rem;grid-column:1/-1">'
        + '<i class="fas fa-music" style="font-size:2rem;opacity:0.3;display:block;margin-bottom:0.5rem"></i>'
        + (msgs[lang]||msgs.es)
        + '<br><a href="creator.html" class="btn btn-primary" style="margin-top:1rem;display:inline-flex"><i class="fas fa-plus"></i> Crear</a></div>';
      return;
    }
    grid.innerHTML = '';
...
// Load genre counts
function loadGenreCounts() {
  ['country','rock','gospel','folk','worship','ballad'].forEach(function(g) {
    fetch('api/songs.php?genre=' + g).then(function(r){return r.json();}).then(function(d) {
      var el = document.getElementById('count-' + g);
      if (el) el.textContent = (d.total || 0) + (d.total===1 ? ' canción' : ' canciones');
    }).catch(function(){});
  });
}
loadGenreCounts();
